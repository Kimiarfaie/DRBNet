...
#### metrics #################################
compute_lpips = lpips.LPIPS(net='alex')  # Remove .cuda()
...
# Add this part to handle custom dataset
if opt.custom_data_path:
    input_c_file_path_list = natsorted(glob(os.path.join(opt.custom_data_path, '*.png')))
    gt_file_path_list = None  # Set this to None if you don't have ground truth images
...
for i, filename in enumerate(input_c_file_path_list):
    ...
    C = torch.FloatTensor(C.transpose(0, 3, 1, 2).copy())
    ...
    if opt.net_mode == 'dual':
        ...
        R, L = torch.FloatTensor(R.transpose(0, 3, 1, 2).copy()), torch.FloatTensor(L.transpose(0, 3, 1, 2).copy())
    if gt_file_path_list is not None:
        GT = crop_image(read_image(gt_file_path_list[i], 255.0))  # here to [0,1]
        GT = torch.FloatTensor(GT.transpose(0, 3, 1, 2).copy())
    ...
    with torch.no_grad():
        if opt.net_mode == 'single':
            ...
            network.load_state_dict(torch.load(opt.ckpt_path, map_location=torch.device('cpu')))
            start_time = time.time()
            output = network(C)
            time_per = time.time() - start_time
        else:
            network = DeblurNet_dual()
...
def prepare_input(resolution):
    input_blur_C = torch.FloatTensor(1, 3, 720, 1280)
    input_blur_L = torch.FloatTensor(1, 3, 720, 1280)
    input_blur_R = torch.FloatTensor(1, 3, 720, 1280)
